       ...
       case 'reset': {
        return [ 
           ...
           .custom((value, { req }) => {
            ...
            .isLength({min: 6}).withMessage('Please enter min 6 characters')
          ]
       }
      default: return null;
    }
...
router.use('/recover', validate('recover'), recover);
router.use('/reset-token', validate('reset-token'), resetToken);
router.use('/reset', validate('reset'), reset);
